...
if __name__ == '__main__':
    wat = cwatchAPI("dan.felts@xerox.com","D@niel01")
    #print wat.getReport("ee43579e-a949-47e8-ad75-838f8328843e")
    #id = wat.getPreReport("Valeant")
    #print wat.getReportQueue("12372","90")
    id = wat.getCompanyID("Valeant")
    print wat.vmserverlist(id)
    #if id == None:
    #    pass
    #else:
    #    ls = wat.report(mtype="list",args="companyid=%s" % id)
    #    for b in ls.iter(tag="Job"):
    #        print b.attrib.get("ID")
    #        for c in b.iter(tag = "Sequence"):
    #            print c.attrib.get("ID")
    #ndata = wat.mssplist()
    #for c in ndata.iter('Company'):
    #    print "%s:%s" % (c.find("CompanyName").text,c.attrib.get("CompanyID"))
    #for c in ndata.iter():
    #949-47e8-ad75-838f8328843e    print c.tag, c.attrib
